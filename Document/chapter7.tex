...
\emph{WorkshopAR} was build with the idea in mind of \emph{augmenting communication}, and this idea was born from the goal of capitalizing on the main advantages that \ac{AR} offers: strong visuals,
...
\begin{itemize}
    \item Unless the technical challenges of a project are the focus of development, there is no need to reinvent the wheel in matters of augmentation, visualization and deployment techniques.
    ...
    offer the needed development backbone to facilitate and quicken development in a way that the focus can be given to design and customization.
    ...
    recognition techniques, are still strongly linked to the capabilities of the deployment hardware. Additionally, \ac{AR} in general is still very resource intensive, and low specs hardware will
    struggle to work, even if its compatible with the development.
    \item Interactions in \ac{AR} (and XR in general) are not standard yet. A lot of interesting techniques and approaches have been proposed, but the default implementation changes from hardware to
...
In the educational front, the first set of guidelines can be focused on the implementation of a collaborative approach. A good portion of the analysis of the case study is dedicated to understanding
...
\begin{itemize}
    ...
    seldom ends in good results.
\end{itemize}

By specifically taking into consideration the case of and AR tool used for education, the major insight that can be given is to be creative. The possibilities around what can be used inside the classroom
...
\begin{itemize}
    \item To manage the amount of distraction and side-tracking that the experience can generate. Self-exploration approaches can minimize this inconvenient, but a clear structure and guidance from the
    educator is always a necessity.
    \item To manage setup and troubleshooting times, they are unavoidable, especially for new developments.
    ...
    inside the classroom and a concept that is hard to visualize or interact with. The focus should not be that much in the visualization but in the connection between the visual elements and the context
...
(now called Niantic Spatial as of 2026) provide a robust development toolkit meant primarily for testing and prototyping, but a more robust solution would be needed for production deployment. Amazon
...
conceptualized as a tool to help in a course in mechanical design. The same tools could be used there, just in a different context. This specialization of \emph{WorkshopAR} could be achieved through modular
